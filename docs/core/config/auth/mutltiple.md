...
directory properly (i.e. virtual users' `home=/var/vmail/%{domain}/%{username}`) and then set
...
userdb sql {
  query = SELECT uid, gid, '/var/vmail/%{domain}/%{username}' AS home FROM users WHERE userid = '%{user}'
}

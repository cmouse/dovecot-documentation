...
passdb sql {
  query = SELECT user_name, domain_name, password \
    FROM users \
    WHERE user_name = '%{username}' AND domain_name = '%{domain}'
}
```
...
passdb sql {
  query = SELECT user_name, domain_name, password \
    FROM users \
    WHERE user_name = '%{username}' AND domain_name = '%{domain}' AND masteradmin='1'
}
```
...
passdb sql {
  query = SELECT user_name, domain_name, password \
    FROM users \
    WHERE user_name = '%{username}' AND domain_name = '%{domain}' AND owns_domain='1' AND '%{domain}'='%{login_domain}'
}
```
...
passdb sql {
  query = SELECT user_name, domain_name, password \
    ...
    WHERE user_name = '%{username}' AND domain_name = '%{domain}' AND login_id='%{user}' AND owned_object='%{login_domain}'
}
```
...
passdb sql {
  query = SELECT user_name, domain_name, password \
    FROM users, ownership \
    WHERE user_name = '%{username}' AND domain_name = '%{domain}' \
      AND ( (masteradmin='1') OR (owns_domain='1' AND '%{domain}'='%{login_domain}') \

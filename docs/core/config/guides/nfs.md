...
* Use [[setting,mail_volatile_path,/dev/shm/dovecot/%{user | md5 | fold(256) | hex(4) }/%{user}]] to store some
  temporary files (e.g. lock files) in tmpfs rather than NFS.
* Use [[setting,mailbox_list_index_prefix,/fast/%{user | md5 | fold(256) | hex(4)}/%{user}]] to use "smaller
...
* Use [[setting,mailbox_list_iter_from_index_dir,/slow/%{user | md5 | fold(256) | hex(4)}/%{user}]] to use

...
* How to replace '%N' in new format:
  ...
  * Example: `%{ user | md5 | fold(256) | hex(4)}` would return maximum 256 different hashes in range
    `00..ff`.

* How to replace '%M' in new format:

  * Example: `%{user | md5 | hex | substr(1)}/%{user | md5 | hex | substr(2,1)}/%{user}` returns directories from `0/0/user` to
    `f/f/user`.

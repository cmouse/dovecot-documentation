...
```[dovecot.conf]
mail_driver = mdbox
mail_path = /var/vmail/%{domain}/%{username}
mail_alt_path = /altstorage/vmail/%{domain}/%{username}
```

will make Dovecot look for message data first under `/var/vmail/%{domain}/%{username}`
("primary storage"), and if it is not found there it will look under
`/altstorage/vmail/%{domain}/%{username}` ("alternate storage") instead. There's no problem

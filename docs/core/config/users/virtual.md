...
usernames (eg. `%{username}` and `%{domain}` [[variable]]), nothing breaks if you use,
...
::: code-group
```[dovecot.conf]
mail_home = /var/vmail/%{domain}/%{username}
mail_driver = maildir
mail_path = ~/mail
```
:::
...
contained %{domain} to specify the domain. If you want the files to contain
...
```[dovecot.conf]
mail_driver = maildir
mail_path = /home/%{domain}/%{username}/Maildir

passdb passwd-file {
  auth_username_format = %Ln
  passwd_file_path = /home/%{domain}/etc/shadow
}

userdb passwd-file {
  auth_username_format = %Ln
  passwd_file_path = /home/%{domain}/etc/passwd
}
```
...
userdb static {
  fields {
    uid = vmail
    gid = vmail
    home = /var/mail/virtual/%{domain}/%{username}
  }
}
```

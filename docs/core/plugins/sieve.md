...
| -------- | --------- | ----------- |
| `%{user}` | `%{user}` | username |
| `%{username}` | `%{username}` | user part in user@domain, same as `%{user}` if there's no domain |
| `%{domain}` | %{domain} | domain part in user@domain, empty if user there's no domain |
...
#   %{user}      - username
#   %{username}      - user part in user@domain, same as %{user} if there's no domain
#   %{domain}      - domain part in user@domain, empty if there's no domain
#   %{name} - name of the Sieve script
...
plugin {
  ...
  sieve_before3 = /var/vmail/%{domain}/%{username}/sieve-before
  ...
  sieve_after = /var/vmail/%{domain}/%{username}/sieve-after
